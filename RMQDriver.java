...
public class RMQDriver {
    ...
    public static void main(String[] args) {
        ...
        if (args.length > 1) {
            rangeSize = (n * Integer.parseInt(args[1]))/ 100;
        }
        ...
        timer = new Timer();
        timer.start();
        rmqSt.preProcess(arr); // run several trials -- see query processing below
        timer.end();
        ...
        for (int i = 0; i < numTrials; i++) {
            ...
            end = begin + rangeSize-1;
            int result = rmqSt.query(arr, begin, end);
            verify(arr, begin, end, result);
            System.out.println("minimum between " + begin + " and " + end + " is " + result);
    ...
    private static void verify(int[] arr, int i, int j, int result) {
        ...
        if (min != result) {
            ...
            System.out.println(" result " + result + " is not actual min " + min);
            System.exit(1);
        }
    }
